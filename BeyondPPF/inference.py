...
def inference(point_cloud:np.ndarray, res:float, receptive_field:float, test_samples:int, test_sample_points:int, num_more:int, 
    ...
    with torch.no_grad():
        ...
        for j in range(J):
            ...
            selected_indices = torch.topk(preds_conf, int(topk * preds_conf.shape[0]), largest=True, sorted=False)[1]

            # # TODO: visualize heatmap
            # vis_conf = preds_conf[selected_indices].cpu().numpy()   # (topk * test_sample_points,)
            # vis_point_idxs = point_idxs[selected_indices].cpu().numpy()   # (topk * test_sample_points, 2)
            # vis_point_heat = np.zeros((pc.shape[0],))               # (N,)
            # vis_point_count = np.zeros((pc.shape[0],))              # (N,)
            # for i in range(vis_conf.shape[0]):
            #     vis_point_heat[vis_point_idxs[i]] += vis_conf[i]
            #     vis_point_count[vis_point_idxs[i]] += 1
            # vis_point_heat = vis_point_heat[vis_point_count > 0]    # (N',)
            # vis_pc = pc[vis_point_count > 0]
            # vis_point_count = vis_point_count[vis_point_count > 0]
            # vis_point_heat /= vis_point_count
            # print(np.max(vis_point_heat), np.min(vis_point_heat))
            # vis_point_heat = (vis_point_heat - np.min(vis_point_heat)) / (np.max(vis_point_heat) - np.min(vis_point_heat))
            # vis_pcd = o3d.geometry.PointCloud()
            # vis_pcd.points = o3d.utility.Vector3dVector(vis_pc)
            # # paint vis_pcd with colormap
            # import matplotlib.cm as cm
            # cmap = cm.get_cmap('jet')
            # vis_pcd.colors = o3d.utility.Vector3dVector(cmap(vis_point_heat)[:, :3])
            # # draw
            # o3d.visualization.draw_geometries([vis_pcd])

            # vote translation
            preds_tr = preds[:, 2*j:2*(j+1)]
            preds_tr = preds_tr[selected_indices, :]
            selected_point_idxs = point_idxs[selected_indices, :]
            
            block_size = (preds_tr.shape[0] + 512 - 1) // 512
            ...
            with cp.cuda.Device(device):
                ...
                ppf_kernel(
                    ...
                    (
                        ...
                        cp.ascontiguousarray(cp.asarray(selected_point_idxs).astype(cp.int32)),
                        grid_obj,
                        cp.ascontiguousarray(cp.asarray(corners[0]).astype(cp.float32)),
                        cp.float32(res),
                        cp.int32(preds_tr.shape[0]),
                        cp.int32(num_rots),
                        grid_obj.shape[0],
                        grid_obj.shape[1],
                        grid_obj.shape[2]
            ...
            preds_conf[torch.topk(preds_conf, int((1 - topk) * preds_conf.shape[0]), largest=False, sorted=False)[1]] = 0
...
if __name__ == '__main__':
    ...
    # load data
    if args.dataset:
        point_cloud = np.load(args.data_path)['point_cloud']
    else:
        point_cloud = np.load(args.data_path)

    # inference
    start_time = time.time()
    results = inference(point_cloud, cfg.shot.res, cfg.shot.receptive_field, cfg.test_samples, cfg.test_sample_points, cfg.num_more, 
    ...
    rgb = np.load(args.data_path)['rgb']
    ...
    for j in range(translations.shape[0]):
        ...
        temp2 = np.cross(directions[j], np.array([1., 0., 0.]))
        if np.linalg.norm(temp2) < 1e-6:
            temp1 = np.cross(np.array([0., 1., 0.]), directions[j])
            temp1 /= np.linalg.norm(temp1)
            temp2 = np.cross(directions[j], temp1)
            temp2 /= np.linalg.norm(temp2)
        else:
            temp2 /= np.linalg.norm(temp2)
            temp1 = np.cross(temp2, directions[j])
            temp1 /= np.linalg.norm(temp1)
        rotation[:, 0] = temp1
        rotation[:, 1] = temp2
        rotation[:, 2] = directions[j]
        joint_axis.rotate(rotation, np.array([[0], [0], [0]]))
        joint_axis.translate(translations[j].reshape((3, 1)))
        ...
        joint_point = joint_point.translate(translations[j].reshape((3, 1)))
